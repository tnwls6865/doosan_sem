...
for epoch in range(opt.epochs):
    ...
    for i, data in enumerate(trainloader, 0):
        optimizer.zero_grad()
        x_categ, x_cont, y_gts, image_feature = data[0].to(device), data[1].to(device),data[2].to(device), data[5].to(device)

        # convert the data to embeddings in the next step
        _ , x_categ_enc, x_cont_enc = embed_data(x_categ, x_cont, model,vision_dset) 
