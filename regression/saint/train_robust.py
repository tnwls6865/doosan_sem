    ...
    if epoch%5==0:
            ...
            with torch.no_grad():
                if opt.task in ['binary','multiclass']:
                    ...
                    #return mae, mse, rmse, msle, rmsle, r2
                    
                    valid_mae,valid_mse,valid_rmse,valid_r2, y_test, y_pred  = mean_sq_error(model, validloader, device,vision_dset)    
                    test_mae,test_mse,test_rmse,test_r2, y_test, y_pred  = mean_sq_error(model, testloader, device, vision_dset)  
                    
                    y_tests.append(y_test.tolist())
                    y_preds.append(y_pred.tolist())

                    #import pandas as pd
                    #df1 = pd.DataFrame(y_tests)
                    #df2 = pd.DataFrame(y_preds)
                    #df = pd.concat([df1, df2], axis=0)
                    #df1.to_excel(r'/home/jungmin/workspace/doosan/saint/output_prediction1.xlsx',index=False)
                    #df2.to_excel(r'/home/jungmin/workspace/doosan/saint/output_prediction2.xlsx',index=False)
                    
                    print('[EPOCH %d] VALID | RMSE: %.3f / MAE: %.3f / MSE: %.3f / R2: %.3f' %(epoch + 1, valid_rmse,valid_mae,valid_mse,valid_r2 ))
                    print('[EPOCH %d] TEST | RMSE: %.3f / MAE: %.3f / MSE: %.3f / R2: %.3f' %(epoch + 1, test_rmse,test_mae,test_mse,test_r2 ))
                    if opt.active_log:
...
plt.scatter(y_tests, y_preds, alpha=0.4)
plt.xlabel("Actual")
